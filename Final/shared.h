...
// Bounds
static const std::size_t MAX_DATACOLS= 1e2; //max number of data columns
...
static const std::size_t MAX_TRAN    = 2e4; //max number of transits
...
static const std::size_t MAX_LYAPNTS= 1e4; //max lyapunov points
...
struct Switches{ // Input switches
    sim_type_t simu= SIM_TRAJ;       //sim type traj/poin/ense/lyap
...
struct Field{ // Input field variables
    ...
    // double     d_potfreq= 1.0;       // pot frequency
    int        i_npot= 1;            // number of waves
    double     d_epot= 0.0;          // pt in keV
    pot_prof_t potp= POT_SIN;       // pt profile
};
...
struct Code{ // Normalized variables
    ...
    double     d_tran = 0.0;        // the lenght of the transit
    double     d_lpert= 0.0;        // perturbation value
};
...
//Lyapunov variables
extern std::vector<std::vector<double>> rcrd_lya;
