...
void inivar(){ //initialize simulation variables
	...
	code.d_pt= // electrostatic potential
		(1.0e-5)*KEV_TO_ERG*field.d_epot/code.d_enax;
...
void set(const int& k){ //set test particle variables
	...
	switch(swt.simu){
		case SIM_LYAP: {
				//code.d_lpert = 1.0e-6;
				//std::uniform_real_distribution<> dist_pert(-code.d_lpert, code.d_lpert);
				//pol[k]= code.d_poli + dist_pert(gen);
				pol[k] = code.d_poli + (0.005*((2.0*urand())-1.0));
				tht[k]= code.d_thet;
				zet[k]= code.d_zeta;
		...
		case SIM_ENSE: {
				...
				tht[k] = code.d_thet;
				zet[k] = code.d_zeta;
				//pol[k]= code.d_poli*((1.0+((2.0 * urand()) - 1.0))/10.0);
				//tht[k]= code.d_thet*((1.0+((2.0 * urand()) - 1.0))/10.0);
				//zet[k]= code.d_zeta*((1.0+((2.0 * urand()) -1.0))/10.0);
				//tht[k]= signnum(0.5 -urand())*M_PI*urand();
				//zet[k]= 2*M_PI*urand();
				ptch[k]= set_pitch(test.ptch_dist);
