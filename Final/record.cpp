...
std::array<double,RCRDMOME_SIZE> rcrd_flux; // record flux points
std::array<double,RCRDMOME_SIZE> rcrd_mome; // record mome points
std::vector<double>              rcrd_poin; // record poin points
std::vector<std::vector<double>> rcrd_lya;  // record lyapunov points
...
void record_LYA(const int& k,const int& skip){ //record traj points
	const double tim_dum= // time in s
		tim[k]/code.d_freq;
	const double rad_dum= // radial position in cm
		sqrt(2*pol[k])*code.d_leng;
	double tht_dum= tht[k];// poloidal pos in radians
	double ntht = floor(tht_dum/(2*M_PI)); // number of poloidal turns
	{ //bound -pi< tht< pi
		tht_dum= fmod(tht_dum,2*M_PI);
		if(tht_dum<-M_PI) tht_dum+= +2*M_PI;
		if(tht_dum>M_PI) tht_dum+= -2*M_PI;
	}
	double zet_dum= zet[k]; // toroidal pos in radians
	double nzet = floor(zet_dum/(2*M_PI)); //number of toroidal  turns
	{ //bound 0< zet< 2*pi
	...
	rcrd_lya[k][0*sim.i_traj +skip]= tim_dum; // time
	rcrd_lya[k][1*sim.i_traj +skip]= rad_dum/field.i_rmin; //radial r/a
	rcrd_lya[k][2*sim.i_traj +skip]= tht_dum; // poloidal pos
	rcrd_lya[k][3*sim.i_traj +skip]= ntht; // poloidal turns
	rcrd_lya[k][4*sim.i_traj +skip]= zet_dum; // toroidal pos
	rcrd_lya[k][5*sim.i_traj +skip]= nzet; // toroidal turns
	// rcrd_lya[k][4*sim.i_traj +skip]= pparl;
	// rcrd_lya[k][5*sim.i_traj +skip]= pperp;
	rcrd_lya[k][6*sim.i_traj +skip]= en[k]*code.d_enax/KEV_TO_ERG; // energy
	rcrd_lya[k][7*sim.i_traj +skip]= ptch[k];  //pitch lambda cos(alpha)
	rcrd_lya[k][8*sim.i_traj +skip]= xdum; // x coordinate
	rcrd_lya[k][9*sim.i_traj +skip]= ydum; // y coordinate
	rcrd_lya[k][10*sim.i_traj +skip]= zdum; // z coordinate
	//rcrd_lya[k][11*sim.i_traj +skip]= k +1;
}

void write_LYA() { // write traj for each particle
    const int numdat = 11; // number of data columns
    std::ofstream LYA;

    for (int k = 0; k < sim.i_part; k++) {
        // Create a filename with particle index
        std::string filename = outdir + "/lya_" + std::to_string(k+1) + ".plt";
        LYA.open(filename, std::ios::app);
        LYA << "#LYA.plt    tim rad tht nt zet nz eng pch x y z\n";
        LYA << std::setprecision(8) << std::scientific;

        for (int i = 0; i < sim.i_traj; i++) {
            for (int j = 0; j < numdat; j++) {
                // Convert specific columns to integer
                if (j == 3 || j == 5)
                    LYA << int(rcrd_lya[k][j * sim.i_traj + i]) << "  ";
                else
                    LYA << rcrd_lya[k][j * sim.i_traj + i] << "  ";
            }
            LYA << "\n";
        }
        LYA.close();
...
// void write_LYA(){ //write traj.plt
// 	const int numdat= 12; //number of data columns
// 	std::ofstream LYA;
// //	std::ofstream(outdir+"/traj.plt");
// 	LYA.open(outdir+"/LYA.plt",std::ios::app);
// 	LYA <<"#LYA.plt    tim rad tht nt zet nz eng pch x y z k\n";
// 	LYA <<std::setprecision(8) <<std::scientific;
// 	for(int i=0;i<sim.i_traj;i++){
// 		for(int k=0;k<sim.i_part;k++){
// 			for(int j=0;j<numdat;j++){
// 				if(j == 11 or j == 3 or j == 5) LYA << int(rcrd_lya[k][j*sim.i_traj +i]) << "  ";
// 				else LYA << rcrd_lya[k][j*sim.i_traj +i] << "  ";
// 			}
// 			LYA << "\n";
// 		}
// 	}
...
void record_traj(const int& k,const int& skip){ //record traj points
	...
	rcrd_traj[3*sim.i_traj +skip]= ntht;
	rcrd_traj[4*sim.i_traj +skip]= zet_dum;
	rcrd_traj[5*sim.i_traj +skip]= nzet;
	// rcrd_traj[4*sim.i_traj +skip]= pparl;
	// rcrd_traj[5*sim.i_traj +skip]= pperp;
	rcrd_traj[6*sim.i_traj +skip]= en[k]*code.d_enax/KEV_TO_ERG;
	rcrd_traj[7*sim.i_traj +skip]= ptch[k];
	rcrd_traj[8*sim.i_traj +skip]= xdum;
	rcrd_traj[9*sim.i_traj +skip]= ydum;
	rcrd_traj[10*sim.i_traj +skip]= zdum;
	//rcrd_traj[11*sim.i_traj +skip]= k;
}
...
//	std::ofstream(outdir+"/traj.plt");
	...
	const int numdat= 11; //number of data columns
	traj <<"#traj.plt    tim rad tht nt zet nz eng pch x y z\n";
	traj <<std::setprecision(8) <<std::scientific;
	for(int i=0;i<sim.i_traj;i++){
		for(int j=0;j<numdat;j++){
			if(j == 3 or j == 5) traj << int(rcrd_traj[j*sim.i_traj +i]) << "  ";
			else traj << rcrd_traj[j*sim.i_traj +i] << "  ";
		}
		traj << "\n";
	}
	traj.close();
...
//	const double plane= 0.54; //rad_ip[int_zet_dum +int_tht_dum];
	const double plane= 0.8;
...
void record_dist(const int& k, const int& skip){ //record distribution
	...
	if(k==0) dist.tim[skip]= tim[k]/code.d_freq;
...
void write_dist(){ //write distribution
	std::array<std::ofstream,MAX_DISTPNTS+1>dist_n;
...
//		std::ofstream(outdir+"/dist_"+std::to_string(i)+".plt");
	...
	for(int i=0;i<sim.i_dist+1;i++){
		dist_n[i] << "dist_"+std::to_string(i)+".plt    time	" << dist.tim[i] <<'\n';
	...
	for(int i=0;i<sim.i_dist+1;i++) dist_n[i].close();
}
