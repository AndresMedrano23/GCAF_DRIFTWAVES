...
void q_currents(const int &k){ // q profile and currents
	...
	ci[k]= // toroidal current
	    ...
	    field.i_rmin*ra*ra*ew/(1.0 +ra*ew*cos(tht[k]))/qf[k]/code.d_leng;
//	const double ci_polft= rap;
//	const double ci_polst= -ra*qf_pol[k]/qf[k];
//	const double ci_poltt= rap*ra*ew*cos(tht[k])/(1.0 +ra*ew*cos(tht[k]));
//	ci_pol[k]= // toroidal current derivative
//		field.i_rmin*ew*(ci_polft+ci_polst-ci_poltt)/qf[k]
//		/(1.0+ra*ew*cos(tht[k]))/code.d_leng;
	const double ci_polst= -pol[k]*qf_pol[k]/qf[k];
    const double ci_poltt= -pol[k]*rap*ew*cos(tht[k])/(1.0 +ra*ew*cos(tht[k]));
    ci_pol[k]= // toroidal current derivative
            field.i_rmin*ew*(1+ci_polst+ci_poltt)/code.d_polw/qf[k]
            /(1.0+ra*ew*cos(tht[k]))/code.d_leng;
}
...
// Gaussian modification with the center at 0.5 and width of 3, so max error A 0.07 and  0.97 inside
// void electrostatic_potential(const int& k){ // electrostatic potential
// 	if(field.potp==POT_SIN){
// 		double phase = field.i_npot*(tht[k] + pol[k]  -std::sqrt(2)*tim[k]);
// 		double center = 0.5;
// 		double width = 3;
// 		double ra = pol[k]/code.d_polw;
// 		double gauss = exp(-((ra-center)*(ra-center))/(2*width*width));
// 		pt[k]= code.d_pt*gauss*sin(phase);
// 		pt_pol[k]= code.d_pt*(gauss*cos(phase) - ((ra-center) /(code.d_polw* (width*width)))*gauss*sin(phase));
// 		pt_tht[k]= code.d_pt*gauss*cos(phase);
// 		pt_zet[k]= 0.0;
// 	} else if(field.potp==POT_SUPER){ // superposition of waves
// 		const int nwm= field.i_npot; // number of waves, maximum
// 		pt[k]= 0.0;
// 		pt_tht[k] =0.0;
// 		pt_pol[k] =0.0;
// 		double center = 0.5;
...
// 		double ra = pol[k]/code.d_polw;
// 		double gauss = exp(-((ra-center)*(ra-center))/(2*width*width));
// 		for(int nw=1;nw<=nwm;nw++){
// 			double ph = 2 * M_PI / nwm;
// 			double phase = tht[k] +pol[k] -std::sqrt(nw+1)*tim[k] + ph;
// 			pt[k] += code.d_pt*gauss*sin(phase);
// 			pt_pol[k] += code.d_pt*(gauss*cos(phase) - ((ra-center) / (code.d_polw* (width*width)))*gauss*sin(phase));
// 			pt_tht[k] += code.d_pt*gauss*cos(phase);
// 		}
// 		pt_zet[k]= 0.0;
// 	} else if(field.potp==POT_RDM){ // random phase potential
// 			const int nwm= field.i_npot; // number of waves, maximum
// 			pt[k]= 0.0;
// 			pt_tht[k] =0.0;
// 			pt_pol[k] =0.0;
...
// 			std::mt19937 gen(rd());
// 			std::uniform_real_distribution<> dist_pi(0.0, 2.0 * M_PI);
// 			double center = 0.5;
// 			double width = 3;
// 			double ra = pol[k]/code.d_polw;
// 			double gauss = exp(-((ra-center)*(ra-center))/(2*width*width));
// 			for(int nw=1;nw<=nwm;nw++){
// 				double ph = dist_pi(gen);
// 				double phase = tht[k] +pol[k] -std::sqrt(nw+1)*tim[k] + ph;
// 				pt[k] += code.d_pt*gauss*sin(phase);
// 				pt_pol[k] += code.d_pt*(gauss*cos(phase) - ((ra-center) / (code.d_polw* (width*width)))*gauss*sin(phase));
// 				pt_tht[k] += code.d_pt*gauss*cos(phase);
...
// // // Modification of the phase, changing the wave front using an error function
// void electrostatic_potential(const int& k){ // electrostatic potential
// 	if(field.potp==POT_SIN){
// 		double center = 0.5;
// 		double width = 0.25;
// 		double ra = pol[k]/code.d_polw;
// 		double phase = tht[k] +(width*code.d_polw*std::sqrt(M_PI/2)*std::erf((ra-center)/(std::sqrt(2)*width)))  -std::sqrt(2)*tim[k];
// 		double gauss = exp(-((ra-center)*(ra-center))/(2*width*width));
// 		pt[k]= code.d_pt*sin(phase);
// 		pt_pol[k]= code.d_pt*cos(phase)*gauss;
// 		pt_tht[k]= code.d_pt*cos(phase);
// 		pt_zet[k]= 0.0;
// 	} else if(field.potp==POT_SUPER){ // superposition of waves
// 		const int nwm= field.i_npot; // number of waves, maximum
// 		pt[k]= 0.0;
...
// 		for(int nw=1;nw<=nwm;nw++){
// 			double ph = 2 * M_PI / nwm;
// 			double phase = tht[k] +(width*code.d_polw*std::sqrt(M_PI/2)*std::erf((ra-center)/(std::sqrt(2)*width)))  -std::sqrt(2)*tim[k] + ph;
// 			pt[k] += code.d_pt*sin(phase);
// 			pt_pol[k] += code.d_pt*cos(phase)*gauss;
// 			pt_tht[k] += code.d_pt*cos(phase);
// 		}
...
// // Modulation with sech2  and polodial dependence	
void electrostatic_potential(const int& k){ // electrostatic potential
	if(field.potp==POT_SIN){
		double center = 0.5;
		double width = 0.4;
		double ra = sqrt(pol[k]/code.d_polw);
		double phase = tht[k]-(100*tim[k]);
		double sech2 = 1.0/(cosh((ra-center)/width)*cosh((ra-center)/width));
		double dsech2 = sech2*tanh((ra-center)/width)*(1/(width*sqrt(pol[k]*code.d_polw)));
		pt[k]= code.d_pt*sech2*sin(phase);
		pt_pol[k]= code.d_pt*dsech2*sin(phase);
		pt_tht[k]= code.d_pt*sech2*cos(phase);
		pt_zet[k]= 0.0;
	} else if(field.potp==POT_SUPER){ // superposition of waves
		const int nwm= field.i_npot; // number of waves, maximum
		pt[k]= 0.0;
		...
		for(int nw=1;nw<=nwm;nw++){
			double ph = 2 * M_PI / nw;
			double phase = tht[k] -(100*nw*tim[k]) + ph;
			pt[k] += code.d_pt*sech2*sin(phase);
			pt_pol[k] += code.d_pt*dsech2*sin(phase);
			pt_tht[k] += code.d_pt*sech2*cos(phase);
		}
