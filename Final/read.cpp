...
void validate(boost::any& v,
	...
	if(dvalues.size()>1) c.coord_second = dvalues[1];
	if(dvalues.size()>2) c.coord_third = dvalues[2];
	v= c;
}
...
void boost_parse(int ac, char* av[]){ // boost library parsing
	...
	command_options.add_options()
		...
		("simulation_type,t",po::value<string>()->required(),
			"simulation type traj/ense/poin/lyap")
	...
	config_field.add_options()
		...
		("field.electrostatic_potential",
			po::value<coordinate>()->multitoken(),
			"[profile \'SIN\' or \'SUPER\' or \'RDM\', electrostatic potential in keV]")
		;
	// Plasma options
	...
	if(vm.count("simulation_type")){ // Assign simulation type
		...
		else if(user_input=="lyap") swt.simu= SIM_LYAP;
	...
	try{ // Store slices file
		if(vm.count("slices")){
			...
			swt.flux= true;
			throw string("Particle flux will be calculated!");
		}
	}
	...
	if(vm.count("field.electrostatic_potential")){ // electrostatic potential
		...
		if(user_input=="SIN") field.potp= POT_SIN;
		else if(user_input=="SUPER") field.potp= POT_SUPER;
		else if(user_input=="RDM") field.potp= POT_RDM;
		else throw string("Invalid potential profile: ")+user_input;
		field.d_epot=
			stod(vm["field.electrostatic_potential"]
			.as<coordinate>().coord_second);
		field.i_npot= 
			stoi(vm["field.electrostatic_potential"]
			.as<coordinate>().coord_third);	
	}
...
T ex_bool(const T& val, const std::string& message) {
	if(val) {throw std::invalid_argument(message);
	} else {return 0;}
}

template <typename T, typename U> // upper bound exception
T ex_upper_bound(const T& val, const U& max, const std::string& message){
	if(val>max){
		throw std::invalid_argument(
			"Invalid "+message+": "+std::to_string(val)
			+". Max is: "+std::to_string(max));
	} else {return 0;}
}

template <typename T, typename U> // lower bound exception
T ex_lower_bound(const T& val, const U& min, const std::string& messege){
	if(val<min){
		throw std::invalid_argument(
...
void check_bounds(){ // Bound input values
	...
	ex_lower_bound(field.i_phas,0,"perturbation_phase");
	ex_lower_bound(field.i_mmod,0,"m_mode");
	ex_lower_bound(field.i_nmod,0,"n_mode");
	ex_lower_bound(field.i_npot,1,"number of waves");
	ex_upper_bound(field.i_npot,1000,"number of waves");
	ex_lower_bound(field.d_epot,0.0,"minimum amplitude");
	// species
...
std::ofstream dlog; // output variable
template <typename T>
...
//	std::ofstream(outdir+"/data.log");
	...
	switch(swt.simu){
		...
		case SIM_LYAP: LOG_VAL("simulation_type","lyap"); break;
	}
	...
	LOG_VAL("field","electrostatic_potential",
		...
		+vm["field.electrostatic_potential"].as<coordinate>().coord_third);
	
	LOG_STR("\n[plasma]");
	LOG_VAL("plasma","temperature",
		vm["plasma.temperature"].as<coordinate>().coord_first +" "
		+vm["plasma.temperature"].as<coordinate>().coord_second);
	LOG_VAL("plasma","density",
		vm["plasma.density"].as<coordinate>().coord_first+" "
		+vm["plasma.density"].as<coordinate>().coord_second);
...
void info(){ //print info
	const char* myinfo =
R"(gcafpp code by Rodrigo Saavedra and edited by Andres Medrano
Email: saavestro@gmail.com
Email: andresmedranoalbarran@gmail.com)";
	std::clog <<myinfo <<"\n";
}
