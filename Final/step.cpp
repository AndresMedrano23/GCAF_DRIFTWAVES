...
void onestep(const int& k){ // integration step
	...
	double en1= 0.5*bf[k]*bf[k]*rho[k]*rho[k] +mu[k]*bf[k] +pt[k];
	...
	en[k]= 0.5*bf[k]*bf[k]*rho[k]*rho[k] +mu[k]*bf[k] +pt[k];
	...
	// //adapt step
	// double en2 = en[k];
	// double w1 = fabs(en2-en1)/en1; //relative energy change
	// const double dele = 1e-4; //the tolerance
	// const double gx = 10e-7; //step change factor
	// if(dele < w1 && code.d_tran*1e-4 <= dt[k]){ //step is reduced
	// 	dt[k] *= 1-gx;
	// }else if(w1 < dele*0.1 &&  dt[k] <= 0.25*code.d_tran){
	// 	dt[k] *= 1+gx;
	// }
