...
void set(const int&);
void logger();
void record_LYA(const int&, const int&);
...
void lya_loop(){ // lyapunov simulation loop
	int numdat = 11; //number of data columns
	rcrd_lya.assign(sim.i_part,std::vector<double>(sim.i_traj*numdat,0.0));
	srand(time(0));
	#pragma omp parallel for
	for(int k=0;k<sim.i_part;k++){ // particle loop
		int skip_lya= 0; // traj record step counter
		int step_time= 0; // time step counter
		for(int i=0;i<sim.i_sdel*sim.i_tran;i++){ // time loop
			onestep(k);
			//update(k);
			if(tim[k]>=skip_lya*code.d_trun/sim.i_traj)
				{record_LYA(k,skip_lya); skip_lya++;}
			step_time++;
		}
...
try{
	...
	switch (swt.simu){
		case SIM_LYAP: {
			std::clog <<"Lyapunov simulation start...\n";
			lya_loop();
			write_LYA();
