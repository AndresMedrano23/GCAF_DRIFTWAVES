...
void electrostatic_potential(const int& k){ // electrostatic potential
	if(field.potp==POT_SIN){
		double phase = field.i_npot*(tht[k]  -tim[k]);
		pt[k]= code.d_pt*sin(phase);
		pt_tht[k]= code.d_pt*cos(phase);
		pt_pol[k]= 0.0;
		pt_zet[k]= 0.0;
	}
	else{ // superposition of waves
		const int nwm= field.i_npot; // number of waves, maximum
		pt[k]= 0.0;
		pt_tht[k] =0.0;
		for(int nw=1;nw<=nwm;nw++){
			pt[k] = pt[k]+(code.d_pt*sin((nw*tht[k])-tim[k]));
			pt_tht[k] = pt_tht[k] + (nw*code.d_pt*cos((nw*tht[k])-tim[k]));
		}
		pt_pol[k]= 0.0;
		pt_zet[k]= 0.0;
